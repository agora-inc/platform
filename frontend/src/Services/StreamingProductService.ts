...
const getProductById = (productId: number, callback: any) => {
  get(`products?id=${productId}`, "", callback);
};

const getStreamingProductById = (productId: number, callback: any) => {
...
const getStreamingProductByFeatures = (
  ...
  get(
    `products/streaming?tier=${tier}&audienceSize=${audienceSize}&productType=${product_type}`,
    "",
    callback
  );
};
