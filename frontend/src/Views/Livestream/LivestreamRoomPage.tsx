...
const AgoraStream: FunctionComponent<Props> = (props) => {
  ...
  const [role, setRole] = useState(
    props.role !== undefined || props.role != "" ? props.role : "audience"
  );

  const [name, setName] = useState("");
  const [storedName, setStoredName] = useState(
    getLocalName(props.talkId.toString()) || ""
  );
  ...
  async function stopTalk() {
    if (
      !window.confirm(
        "Are you sure? Doing so will redirect people to the Cafeteria."
      )
    ) {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (role == "admin" || role == "speaker") {
      let request_unsubs = FirebaseDb.collection("requests")
      ...
      let request_unsubs = FirebaseDb.collection("requests")
        .where("requester_id", "==", localUser.uid)
        .onSnapshot((snaps) => {
          let req = snaps.docs
            .filter((d) => d.exists)
            .map((d) => {
            ...
            .filter((d) => d.requester_id === localUser.uid)
            .find((d) => d.status === "GRANTED" || d.status === "REQUESTED");
  ...
  function requestMicButton() {
    return (
      ...
      >
        <Text weight="bold" color="white" size="14px" textAlign="center">
          {hasMicRequested || !callControl.mic
            ? "Give up microphone"
            : "Request microphone"}
        </Text>
      </Box>
  ...
  function requestMicBox() {
    return (
      <>
        <Text size="16px" color="grey" style={{ marginBottom: "10px" }}>
          Requests for mic
        </Text>
        <Table>
          <TableBody>
            {micRequests.map((req, i) => (
              <TableRow key={i} className="request-item">
                ...
                >
                  <Text size="14px" weight="bold">
                    {" "}
                    {req.status == "GRANTED" ? "Remove" : "Refuse"}{" "}
                  </Text>
                </Button>
              </TableRow>
            ))}
              ...
              >
                ...
                {(callControl.slideShare || isSlideVisible) && (
                  <PDFViewer
                    ...
                    presenter={callControl.slideShare}
                  />
                )}
              </Box>
            </Box>
