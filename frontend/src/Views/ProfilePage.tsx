...
const ProfilePage = (props: Props) => {
  ...
  useEffect(() => {
    user && ProfileService.getProfile(user.id, setProfile);
  }, []);
  ...
  const getProfilePhotoUrl = () => {
    if (profile) {
      return ProfileService.getProfilePhotoUrl(profile.user.id);
    } else {
      return "";
    }
  };

  const onProfilePhotoUpload = async (file: File) => {
    const token = await getAccessTokenSilently();
    if (home && profile) {
  ...
  const addNewPresentation = (new_presentation: Presentation) => {
    setPresentations([...presentations, new_presentation]);
  };

  const updatePaper = (index: number, new_paper: Paper) => {
    const temp = papers;
  ...
  const postTopics = async (topics: Topic[]) => {
    const token = await getAccessTokenSilently();
    if (profile) {
  ...
  const defaultBio = () => {
    if (profile) {
      return (
        profile.full_name +
        " is a young professor who has recently been getting a lot of traction and visibility within the community, thanks to their lattest work. Many researchers in the field have been speculating his nomination as a recipient of the next Nobel price."
      );
    } else {
      return "Insert your bio";
    }
  };
    ...
    return (
      <>
        ...
        <Box
          ...
          pad="small"
        >
          <Box direction="row" height="150px" align="center" justify="between">
          ...
          <BioEntry
            bio={profile.user.bio ? profile.user.bio : defaultBio()}
            home={home}
          />
            ...
            <Tabs>
              <TabList>
                <Tab>
                  ...
                  >
                    <Workshop />
                    <Text size="14px">Talk applications</Text>
                  </Box>
                </Tab>
                {home && (
                  <Tab>
              ...
              <TabPanel style={{ width: "78vw", minHeight: "800px" }}>
                ...
                {presentations.length === 0 && (
                  ...
                  >
                    No pending talk application. Click on "Get invited to speak"
                    to create one!
                  </Text>
                )}
              </TabPanel>
                ...
                <TabPanel style={{ width: "78vw", minHeight: "800px" }}>
                  ...
                  >
                    ...
                    {adminChannels.length === 0 && (
                      ...
                      >
                        An agora is where seminar organizers publish the next
                        talks, upload the recordings of the previous seminars,
                  ...
                  <Box focusIndicator={false} gap="xsmall">
                    <Text size="14px" weight="bold">
                      Following
                    </Text>
                    <Box
              ...
              <Box direction="column" gap="30px">
                <TagsEntry tags={profile.tags} home={home} hasTitle={true} />
