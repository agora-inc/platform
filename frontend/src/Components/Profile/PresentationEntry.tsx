...
export const PresentationEntry = (props: Props) => {
  ...
  const [id, setId] = useState(props.presentation.id);
  const [userId, setUserId] = useState(props.presentation.user_id);
  ...
  const [nDaysLeft, setNDaysLeft] = useState(-1);
  ...
  const updatePresentation = async (renew: boolean = false) => {
    let now = new Date();
    if (props.home && props.updatePresentation && user) {
  ...
  const deletePresentation = async () => {
    if (props.home && props.deletePresentation) {
      ...
      ProfileService.deletePresentation(id, () => {}, token);
    }
  };
