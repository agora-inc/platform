...
export default class CoffeeHangoutButton extends Component<Props, State> {
  ...
  computeCafeteriaOpeningTime = () => {
    ...
    return moment(d).subtract(45, "minutes").toDate();
  };

  computeCafeteriaClosingTime = () => {
    let d = new Date(this.props.talk.end_date);
    return moment(d).add(60, "minutes").toDate();
  };
  ...
  showTimeUntil = () => {
    ...
    let secondsUntil = Math.floor(
      (startTime - this.state.now.getTime()) / 1000 -
        this.state.openingTimeBeforeSemInMinutes * 60
    );
  ...
  cafeteriaOpened() {
    return (
      this.state.now > this.state.cafeteriaOpeningTime &&
      this.state.now < this.state.cafeteriaClosingTime
    );
  }
  ...
  cafeteriaPermanentlyClosed() {
    ...
    var secondsBeforeSeminar = Math.floor(
      (startTime - this.state.now.getTime()) / 1000
    );
              ...
              <Box align="center" data-tip data-for="grab_coffee_button_before">
                {!this.props.disabled && (
                  <a
                    ...
                    href={
                      "https://gather.town/app/q9m3D0XU6stq8fNG/morastream%20Cafeteria"
                    }
                    target="_blank"
                  >
                    <Button
                ...
                <ReactTooltip
                  id="grab_coffee_button_before"
                  effect="solid"
                  place="bottom"
                >
                  Chat with other seminar participants and speakers; cafeteria
                  remains open 2 hours after the end of the seminar.
                </ReactTooltip>
              </Box>
            )}
            {this.cafeteriaOpened() || (
        ...
        {this.cafeteriaPermanentlyClosed() && (
          <>
            <Box
              align="center"
              data-tip
              data-for="grab_coffee_button_cafet_perma_closed"
            >
              <Box
                onClick={() => {}}
                background="#0C385B"
            ...
            <ReactTooltip
              id="grab_coffee_button_cafet_perma_closed"
              effect="solid"
              place="bottom"
            >
              The virtual cafeteria opens 45 minutes before the start of a
              seminar and closes 2 hours after it ends.
            </ReactTooltip>
          </>
        )}
      </>
