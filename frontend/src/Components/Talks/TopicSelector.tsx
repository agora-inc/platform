...
export default class TopicSelector extends Component<Props, State> {
  ...
  getChildren = (id: number) => {
    return this.state.all
      .filter(function (topic: any) {
        return (
          topic.parent_1_id == id ||
          topic.parent_2_id == id ||
          topic.parent_3_id == id
        );
      })
      .map((topic: any) => topic.field);
  };
        ...
        >
          {this.state.topicsBeingShown[choiceNumber] >= 0 && (
            <Select
              ...
              onChange={({ option }) =>
                this.onFieldChoose(choiceNumber, this.nameToId(option), 0)
              }
            />
          )}
          {this.state.topicsBeingShown[choiceNumber] >= 1 && (
            <Select
              ...
              onChange={({ option }) =>
                this.onFieldChoose(choiceNumber, this.nameToId(option), 1)
              }
            />
          )}
          {this.state.topicsBeingShown[choiceNumber] >= 2 && (
            <Select
              ...
              onChange={({ option }) =>
                this.onFieldChoose(choiceNumber, this.nameToId(option), 2)
              }
            />
          )}
        </Box>
